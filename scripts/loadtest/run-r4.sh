...
OUT="docs/loadtest/runs/$(date +%Y-%m-%d)-r4"
mkdir -p "$OUT"/{k6,metrics,db}

# R4: R3에서 p95가 전부 30s 타임아웃 천장에 걸려 무효였다. 실측 지속 처리량(~20/s) 기준으로
# 도착률을 낮춰 knee 구간을 실제로 관측한다. VERDICT.md 참고.
HR_STAGES='[{"duration":"1m","target":2},{"duration":"2m","target":5},{"duration":"2m","target":10},{"duration":"2m","target":20},{"duration":"2m","target":30},{"duration":"1m","target":45}]'
FS_STAGES='[{"duration":"30s","target":10},{"duration":"10s","target":60},{"duration":"1m","target":60},{"duration":"20s","target":20},{"duration":"20s","target":0}]'
log() { echo -e "\n=== [$(date -u +%H:%M:%S)] $* ==="; }
