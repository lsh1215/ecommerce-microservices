...
| 역할 | 노드 수 | 머신 타입 | 배치 대상 |
|---|---:|---|---|
| 서비스 | 4 | `e2-standard-2` | Product, Order, Payment, Customer WAS |
| DB | 1 | `e2-standard-4` | mysql-product, mysql-order, mysql-payment, mysql-customer |
| Kafka | 1 | `e2-standard-2` | kafka-0 single broker |
| 모니터링/인프라 | 1 | `e2-standard-4` | LGTM Stack, Traefik, k6 job, mysqld-exporter |
...
| mysql-product/order | `500m / 768Mi` | `1200m / 1536Mi` | 주문 생성 write path 핵심 DB |
| mysql-payment/customer | `300m / 512Mi` | `800m / 1024Mi` | 상대적으로 낮은 write path 기준 |
